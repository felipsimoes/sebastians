...
function getSpectrum() {
  ...
  let spectrum = createImage(width, height);
  
  spectrum.loadPixels();
  row = -1;
  for (let y = 0; y < spectrum.height * d; y++) {
    for (let x = 0; x < spectrum.width * d; x++) {
      ...
      let index = (x + y * width) * 4 * d;
      spectrum.pixels[index + 0] = r;
      spectrum.pixels[index + 1] = g;
      spectrum.pixels[index + 2] = b;
    }
